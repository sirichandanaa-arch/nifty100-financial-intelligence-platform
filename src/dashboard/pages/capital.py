...
def render():
    """Render the Capital Allocation screen."""
    ...
    try:
        ...
        elif "capital"=="peers": st.dataframe(pd.read_sql_query("SELECT * FROM peer_percentiles",con).head(100),use_container_width=True)
        elif "capital"=="trends": st.dataframe(pd.read_sql_query("SELECT * FROM financial_ratios ORDER BY year",con).head(100),use_container_width=True)
        elif "capital"=="sectors": st.dataframe(pd.read_sql_query("SELECT * FROM sectors",con),use_container_width=True)
        elif "capital"=="capital": st.dataframe(pd.read_csv("output/capital_allocation.csv"),use_container_width=True)
